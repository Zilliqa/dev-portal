...
10. Now it's time to synchronize the node with the network. For networks created using Zilliqa 2, the node can be synchronized from the genesis. However, for networks such as mainnet and testnet that migrated from Zilliqa 1, the node must be synchronized from a checkpoint:
  ...
  > **Note on Node Synchronization Reliability**
  > 
  > Starting from version [CURRENT_VERSION], the node's synchronization process has been enhanced to handle potential database insertion conflicts more robustly. Previously, `INSERT OR IGNORE` could lead to unexpected synchronization behavior. Now, the sync module explicitly manages database insertion errors, ensuring that nodes can continue synchronizing even if duplicate block insertions are attempted.
  > 
  > This improvement prevents nodes from getting stuck during network synchronization and provides more stable block processing, especially in high-concurrency or network-intensive scenarios.
