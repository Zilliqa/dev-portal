...
To configure a node and join a Zilliqa 2.0 network, we provide the `z2` utility as part of the [zq2](https://github.com/Zilliqa/zq2/blob/main/) codebase. Follow the step by step guide to setup your node:
...
10. Now it's time to synchronize the node with the network. For networks created using Zilliqa 2, the node can be synchronized from the genesis. However, for networks such as mainnet and testnet that migrated from Zilliqa 1, the node must be synchronized from a checkpoint:
    ...
    > **Note on Block Synchronization Reliability**
    > 
    > The node's block synchronization process has been enhanced to handle potential database insertion conflicts more gracefully. If a block already exists in the database, the synchronization will continue without interruption, preventing 'stuck-node' scenarios during network catch-up or recovery.
