...
This is a breaking change for nodes on v0.19.0 and later. New nodes can only consume `.ckpt` files, while older versions are limited to `.dat` files. Node operators must use the `convert-ckpt` utility to convert any existing `.dat` checkpoints to the new `.ckpt` format before they can be used with the updated node software.
:::
...
2.  **Download the Checkpoint File**
    ...
    -   Look for the `<key>` tag, which contains the checkpoint file's name. The file follows the `block_num.ckpt` format (e.g., `0000291600.ckpt`).
...
3.  **Introduce the New Checkpoint Format**

    The new checkpoint format is a ZIP64-based archive (`.ckpt`) designed for enhanced efficiency and flexibility. Each `.ckpt` archive contains the following components:

    -   `metadata.json`: Contains essential metadata about the checkpoint.
    -   `block.bincode`: Stores block-related data.
    -   `parent.bincode`: Contains information about parent blocks.
    -   `transactions.bincode`: Holds transaction data.
    -   `state.bincode`: Encapsulates the state data.
    -   `history.bincode`: A new component that stores historical data, enabling more comprehensive state reconstruction.

4.  **Conversion Utility**

    Node operators must use the `convert-ckpt` utility to convert any existing `.dat` checkpoints to the new `.ckpt` format before they can be used with the updated node software. This utility ensures compatibility and proper functioning with nodes running v0.19.0 and later.

5.  **Configure Checkpoints in the Configuration File**
    ...
    file = "xxxxxx..." # File name of the checkpoint block. for eg: 3000.ckpt
